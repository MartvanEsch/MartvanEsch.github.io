let contentArr = window.contentArr;

let projectsContainer = document.getElementById('artwork');
...
contentArr.forEach((project) => {
  ...
  projects += 
      `<div class="${project.class}">
        <a class="flex align-center justify-center column gap-small" href="project.html#${project.key}" style="background: url(${project.imgArray[0]});">
          <h3>${project.header}</h3>
          <p>${shorten(project.body, 50)}</p>
        </a>
      </div>`
});
projectsContainer.innerHTML = projects;
  ...
  let filters = []
  contentArr.forEach((project) => {
    if (!filters.includes(project.class)) {
      filters.push(project.class);  
    }
  })
  let filterChipsContainer = document.querySelector('md-chip-set');
  filters.forEach((filter) => {
    let chip = document.createElement('md-filter-chip');
